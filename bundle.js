...
/***/ function(module, exports, __webpack_require__) {
	...
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(1),__webpack_require__(2)], __WEBPACK_AMD_DEFINE_RESULT__ = (function($,grph) { 
	...
	/*
	    var Poss = React.CreateClass({
	        render: function() {
	            var isok = this.props.model[this.props.col][this.props.row];
	            var cls = 'posscell';
	            if (isok) {
	                cls = cls + ' isposs';
	            }
	            ...
	            return(
	                <div className={ 'posscell' + (v === PossVal.)
	            );
	        }
	...
	*/
	    ...
	    var Pkr = React.CreateClass({
	        render: function() { 
	            return (
	                React.DOM.div({className: "pkr-cell", backgroundColor: this.props.clr}, 
	                    this.props.idx
	                )
	            );
	        }
	    ...
	    var Board = React.CreateClass({
	        ...
	        render: function() {
	            var pkrnodes =
	                vals.map( function(i) {
	                    return (
	                        Pkr({idx: i, clr: this.state.pickers[i] ? "Red" : "Black"})
	                    );
	                });
	            return (
	                React.DOM.div({className: "pkr"}, 
	                    pkrnodes
	                )  
	            );
	        }
	    ...
	    var loadBoard = function(board) { 
	        ...
	        var g = grph.Graphics(canvas);
	        var d = grph.Drawer({board: board});

	        d.drawLayout(g);
	        var b = document.getElementById("brd") ;
	        b.style.background = "url(" + canvas.toDataURL() + ")";
	        b.style.backgroundRepeat = "no-repeat";
	        canvas.style.display = "none";
	        var brd = $("#brd");
	        React.renderComponent(Board(null), b);
	        soln = board.solution;

	    };

	    $.get("/puzzles/20140523.gtk.json", function(board) {
	        loadBoard(board); 
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _pen = function(spec) {
	    ...
	    that.dashSpaceSize = spec.dashSpaceSize || 0;
	    return that;
	};
	...
	var _graphics = function(canvas) {
	    ...
	    that.drawLine = function(pen,p1,p2)  {
	        ...
	        if (pen.dashSize > 0) {
	            if (pen.dashSpaceSize > 0) {
	                cxt.setLineDash([pen.dashSize,pen.dashSpaceSize]);
	            } else {
	                cxt.setLineDash([pen.dashSize]);
	            };
	        };
	    ...
	    that.fillRect = function(t,l,w,h,color) {
	        cxt.fillStyle = color;
	        cxt.fillRect(t,l,w,h);
	    };
	...
	var _drawer = function(spec) {
	    var that = {};
	    spec = spec || {};
	    /*
	    var unitsize = spec.unitsize || 72;
	    var boardSize  = spec.boardSize || 9;
	    var backColor = spec.backColor || 'White';
	    var foreColor = spec.foreColor || 'Black';
	    var regtotFont = spec.regtotFont || '10px Verdana Bold';
	    var totFontHeight = spec.totFontHeight || 11;
	    var symFont = spec.symFont || '30px Arial';
	    var symFontHeight = spec.symFontHeight || 20;
	*/
	    var unitsize = spec.unitsize || 36;
	    var boardSize  = spec.boardSize || 9;
	    var backColor = spec.backColor || 'White';
	    var foreColor = spec.foreColor || 'Black';
	    var regtotFont = spec.regtotFont || '7px Verdana Bold';
	    var totFontHeight = spec.totFontHeight || 8;
	    var symFont = spec.symFont || '20px Arial';
	    var symFontHeight = spec.symFontHeight || 14;
	    var bsz = boardSize - 1;
	...
	//        if boardSize = 21 then
	//            let left = unitsize * 9 
	//            let rect = new Rectangle(1 + left, 0, (unitsize * 3) - 1, (unitsize * 6)  )
	//            g.FillRectangle(new SolidBrush(Color.White),rect)
	//            let rect = new Rectangle(1 + left, (unitsize * 15) + 1, (unitsize * 3) - 1, (unitsize * 6)  )
	//            g.FillRectangle(new SolidBrush(Color.White),rect)
	//
	//            let top = unitsize * 9
	//            let rect = new Rectangle(0, top + 1, (unitsize * 6), (unitsize * 3) - 1 )
	//            g.FillRectangle(new SolidBrush(Color.White),rect)
	//            let rect = new Rectangle((unitsize * 15) + 1, top + 1, (unitsize * 6) , (unitsize * 3) - 1 )
	//            g.FillRectangle(new SolidBrush(Color.White),rect)

	        var board = spec.board;

	        board.regions.forEach( function(r) { that.drawRegionBorders(g,r); } );
	    ...
	    that.drawRegionBorders = function(g, reg) {
	        ...
	        if (reg.cells.length > 1) {
	            var bp = _pen({color: "LightGray",dashSize: 5});
	            var nb = 3;
	            ...
	            if (reg.total > 0) {
	                ...
	                var top = nY * unitsize + 1;
	                var left = nX * unitsize + 6; 
	                var s = reg.total.toString();

	                var w = g.measureText(s, regtotFont);
	                g.fillRect(left - 2,top, w + 4, totFontHeight + 2, backColor);
	                g.drawString(s, regtotFont, foreColor, [left,top+totFontHeight - 1]);
	    ...
	    that.drawEqualToSign = function( g, nX, nY) {
	        g.drawString("=",symFont,foreColor,[nX-3,nY+10]);
	    };
	        
	    that.drawVerticalEqualToSign = function( g, nX, nY) {
	        g.drawVerticalString("=",symFont,foreColor,[nX-6,nY-1]);
	    };
	    ...
	    that.drawVerticalLessThanSign = function( g, nX, nY) {
	        g.drawVerticalString("<",symFont,foreColor,[nX-10,nY-8]);
	    };
	...
	return {Pen: _pen, Graphics: _graphics, Drawer: _drawer};
